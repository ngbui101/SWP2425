...
const uint8_t bma456_config_file[] PROGMEM= {
 ...
 *  @return none
 */
static void update_stepcounter_parameter(const struct bma456_stepcounter_settings *setting, uint8_t index,
					uint8_t *feature_config);
/*!
 *  @brief This API copy the settings of step counter into the
 *  structure of bma456_stepcounter_settings, which is read from sensor.
 *
 ...
 *  @return none
 */
static void extract_stepcounter_parameter(struct bma456_stepcounter_settings *setting, const uint16_t *data_p);
...
uint16_t bma456_write_config_file(struct bma4_dev *dev)
{
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if ((dev->read_write_len >= BMA456_RD_WR_MIN_LEN) &&
				(dev->read_write_len <= BMA456_FEATURE_SIZE)) {
				/* Even or odd check */
...
{
	uint8_t feature_config[BMA456_ANYMOTION_EN_LEN + 2] = {0};
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			rslt = bma4_read_regs(BMA4_FEATURE_CONFIG_ADDR, feature_config,
						BMA456_ANYMOTION_EN_LEN + 2, dev);
			if (rslt == BMA4_OK) {
				feature_config[index] = BMA4_SET_BITSLICE(feature_config[index],
									BMA456_ANY_NO_MOTION_AXIS_EN, axis);
				rslt |= bma4_write_regs(BMA4_FEATURE_CONFIG_ADDR, feature_config,
							BMA456_ANYMOTION_EN_LEN + 2, dev);
			}
		} else {
...
{
	uint8_t feature_config[BMA456_ANYMOTION_EN_LEN + 2] = {0};
	uint8_t index = BMA456_ANY_NO_MOTION_OFFSET;
		...
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				feature_config[index++] |= (uint8_t)
					(any_motion->nomotion_sel << BMA456_ANY_NO_MOTION_SEL_POS);

				/* Extract duration */
				duration_lsb = feature_config[index];
				duration_msb = feature_config[index + 1] << 8;
				duration = duration_lsb | duration_msb;
				duration = BMA4_SET_BITS_POS_0(duration, BMA456_ANY_NO_MOTION_DUR,
						any_motion->duration);
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				any_motion->threshold = (*data_p) & BMA456_ANY_NO_MOTION_THRES_MSK;
				/* Extract threshold & nomotion
				selection data */
				anymotion = ((uint8_t)(*(data_p++) >> 8)) & BMA456_ANY_NO_MOTION_SEL_MSK;
				/* Extract no motion field */
				any_motion->nomotion_sel = anymotion >>
					BMA456_ANY_NO_MOTION_SEL_POS;
				/* Extract duration value */
				any_motion->duration = (*(data_p)) & BMA456_ANY_NO_MOTION_DUR_MSK;
			}
		} else {
			rslt = BMA4_E_INVALID_SENSOR;
		}
...
uint16_t bma456_step_detector_enable(uint8_t enable, struct bma4_dev *dev)
{
	...
	uint8_t index = BMA456_STEP_CNTR_OFFSET + 1;
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				feature_config[index] = BMA4_SET_BITSLICE(feature_config[index],
					BMA456_STEP_DETECTOR_EN, enable);
				rslt |= bma4_write_regs(BMA4_FEATURE_CONFIG_ADDR, feature_config,
					BMA456_FEATURE_SIZE, dev);
			}
		} else {
...
{
	uint8_t feature_config[BMA456_FEATURE_SIZE] = {0};
	uint8_t index = BMA456_STEP_CNTR_OFFSET;
	uint16_t wm_lsb = 0;
	uint16_t wm_msb = 0;
	uint16_t rslt = BMA4_OK;
	uint16_t data = 0;
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				16 bits of data */
				data = BMA4_SET_BITS_POS_0(data, BMA456_STEP_CNTR_WM,
					step_counter_wm);
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				wm_msb = feature_config[index + 1] << 8;
				data = wm_lsb | wm_msb;
				*step_counter_wm = BMA4_GET_BITS_POS_0(data, BMA456_STEP_CNTR_WM);
			}
		} else {
			rslt = BMA4_E_INVALID_SENSOR;
		}
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			rslt = bma4_read_regs(BMA4_STEP_CNT_OUT_0_ADDR, data,
					BMA456_STEP_CNTR_DATA_SIZE, dev);
...
{
	uint16_t rslt = BMA4_OK;
	struct bma456_stepcounter_settings sc_settings = {0};

	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			switch (platform) {
			case BMA456_PHONE_CONFIG:
				sc_settings.param1  = BMA456_PHONE_SC_PARAM_1;
				sc_settings.param2  = BMA456_PHONE_SC_PARAM_2;
				sc_settings.param3  = BMA456_PHONE_SC_PARAM_3;
				sc_settings.param4  = BMA456_PHONE_SC_PARAM_4;
				sc_settings.param5  = BMA456_PHONE_SC_PARAM_5;
				sc_settings.param6  = BMA456_PHONE_SC_PARAM_6;
				sc_settings.param7  = BMA456_PHONE_SC_PARAM_7;
				sc_settings.param8  = BMA456_PHONE_SC_PARAM_8;
				sc_settings.param9  = BMA456_PHONE_SC_PARAM_9;
				sc_settings.param10 = BMA456_PHONE_SC_PARAM_10;
				sc_settings.param11 = BMA456_PHONE_SC_PARAM_11;
				sc_settings.param12 = BMA456_PHONE_SC_PARAM_12;
				sc_settings.param13 = BMA456_PHONE_SC_PARAM_13;
				sc_settings.param14 = BMA456_PHONE_SC_PARAM_14;
				sc_settings.param15 = BMA456_PHONE_SC_PARAM_15;
				sc_settings.param16 = BMA456_PHONE_SC_PARAM_16;
				sc_settings.param17 = BMA456_PHONE_SC_PARAM_17;
				sc_settings.param18 = BMA456_PHONE_SC_PARAM_18;
				sc_settings.param19 = BMA456_PHONE_SC_PARAM_19;
				sc_settings.param20 = BMA456_PHONE_SC_PARAM_20;
				sc_settings.param21 = BMA456_PHONE_SC_PARAM_21;
				sc_settings.param22 = BMA456_PHONE_SC_PARAM_22;
				sc_settings.param23 = BMA456_PHONE_SC_PARAM_23;
				sc_settings.param24 = BMA456_PHONE_SC_PARAM_24;
				sc_settings.param25 = BMA456_PHONE_SC_PARAM_25;
				break;

			case BMA456_WRIST_CONFIG:
				sc_settings.param1  = BMA456_WRIST_SC_PARAM_1;
				sc_settings.param2  = BMA456_WRIST_SC_PARAM_2;
				sc_settings.param3  = BMA456_WRIST_SC_PARAM_3;
				sc_settings.param4  = BMA456_WRIST_SC_PARAM_4;
				sc_settings.param5  = BMA456_WRIST_SC_PARAM_5;
				sc_settings.param6  = BMA456_WRIST_SC_PARAM_6;
				sc_settings.param7  = BMA456_WRIST_SC_PARAM_7;
				sc_settings.param8  = BMA456_WRIST_SC_PARAM_8;
				sc_settings.param9  = BMA456_WRIST_SC_PARAM_9;
				sc_settings.param10 = BMA456_WRIST_SC_PARAM_10;
				sc_settings.param11 = BMA456_WRIST_SC_PARAM_11;
				sc_settings.param12 = BMA456_WRIST_SC_PARAM_12;
				sc_settings.param13 = BMA456_WRIST_SC_PARAM_13;
				sc_settings.param14 = BMA456_WRIST_SC_PARAM_14;
				sc_settings.param15 = BMA456_WRIST_SC_PARAM_15;
				sc_settings.param16 = BMA456_WRIST_SC_PARAM_16;
				sc_settings.param17 = BMA456_WRIST_SC_PARAM_17;
				sc_settings.param18 = BMA456_WRIST_SC_PARAM_18;
				sc_settings.param19 = BMA456_WRIST_SC_PARAM_19;
				sc_settings.param20 = BMA456_WRIST_SC_PARAM_20;
				sc_settings.param21 = BMA456_WRIST_SC_PARAM_21;
				sc_settings.param22 = BMA456_WRIST_SC_PARAM_22;
				sc_settings.param23 = BMA456_WRIST_SC_PARAM_23;
				sc_settings.param24 = BMA456_WRIST_SC_PARAM_24;
				sc_settings.param25 = BMA456_WRIST_SC_PARAM_25;
				break;
	...
	if (rslt == BMA4_OK) {
		...
		rslt = bma456_stepcounter_set_parameter(&sc_settings, dev);
	}
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				data_p = data_p + BMA456_STEP_CNTR_PARAM_OFFSET/2;
...
{
	uint8_t feature_config[BMA456_FEATURE_SIZE] = {0};
	uint8_t index = BMA456_STEP_CNTR_PARAM_OFFSET;
	uint16_t rslt = BMA4_OK;
...
{
	uint8_t feature_config[BMA456_FEATURE_SIZE] = {0};
	uint8_t index = BMA456_WAKEUP_OFFSET;
	uint16_t rslt = BMA4_OK;
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				feature_config[index] = BMA4_SET_BITSLICE(feature_config[index],
							BMA456_WAKEUP_SENS, sensitivity);
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				*sensitivity = BMA4_GET_BITSLICE(feature_config[index], BMA456_WAKEUP_SENS);
			}
		} else {
			rslt = BMA4_E_INVALID_SENSOR;
		}
...
static void update_len(uint8_t *len, uint8_t feature, uint8_t enable)
{
	uint8_t length = BMA456_FEATURE_SIZE;

	if ((feature == BMA456_ANY_MOTION) || (feature == BMA456_NO_MOTION)) {
		/* Change the feature length to 2 for reading and writing of 2 bytes for
		any/no-motion enable */
		length = BMA456_ANYMOTION_EN_LEN;
...
static uint16_t feature_enable(uint8_t feature, uint8_t len, uint8_t *feature_config, struct bma4_dev *dev)
{
	...
	/* Enable activity */
	if ((feature & BMA456_ACTIVITY) > 0) {
		/* Activity enable bit pos. is 1 byte ahead of the
		base address */
		index = BMA456_STEP_CNTR_OFFSET + 1;
		feature_config[index] = feature_config[index] | BMA456_ACTIVITY_EN_MSK;
	}

	/* Enable wakeup */
	if ((feature & BMA456_WAKEUP) > 0) {
		/* Wakeup enable bit pos. is the base address(0x38) of wakeup*/
		index = BMA456_WAKEUP_OFFSET;
		feature_config[index] = feature_config[index] | BMA456_WAKEUP_EN_MSK;
	}

	/* Enable wrist-tilt */
	if ((feature & BMA456_WRIST_TILT) > 0) {
		/* Tilt enable bit pos. is the base address(0x3A) of tilt */
		index = BMA456_WRIST_TILT_OFFSET;
		feature_config[index] = feature_config[index] | BMA456_WRIST_TILT_EN_MSK;
	}

	/* Enable anymotion/nomotion */
	if ((feature & BMA456_ANY_MOTION) > 0 || (feature & BMA456_NO_MOTION) > 0) {
		/* Any/Nomotion enable bit pos. is 1 bytes ahead of the
		any/nomotion base address(0x00) */
		index = 1;

		if ((feature & BMA456_ANY_MOTION) > 0) {
			/* Enable anymotion */
			feature_config[index] = feature_config[index] & (~BMA456_ANY_NO_MOTION_SEL_MSK);
		} else {
			/* Enable nomotion */
			feature_config[index] = feature_config[index] | BMA456_ANY_NO_MOTION_SEL_MSK;
		}
...
static uint16_t feature_disable(uint8_t feature, uint8_t len, uint8_t *feature_config, struct bma4_dev *dev)
{
	...
	if ((feature & BMA456_ANY_MOTION) > 0 || (feature & BMA456_NO_MOTION) > 0) {
		...
		any/nomotion base address(0x00) */
		index = 3;
		feature_config[index] = feature_config[index] & (~BMA456_ANY_NO_MOTION_AXIS_EN_MSK);
	}
